...
- `test-flashdoc.html` is the manual regression fixture for selection parsing and notification flows.
...
- `zip -r out/flashdoc.zip . -x 'out/*' '.git/*'` packages the extension for Web Store upload.
- `python3 -m http.server 8080` serves fixtures such as `test-flashdoc.html` locally for manual checks.
...
- Keep functions camelCase, DOM classes kebab-case, and default download paths under `FlashDocs/`.

## Testing Guidelines
- No automated suite exists; exercise save flows via context menu, keyboard shortcut, and popup actions after each change.
- Validate smart type detection using YAML, Python, JSON, and Markdown snippets from `test-flashdoc.html`.
