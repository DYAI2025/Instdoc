...
- **Consistent naming and folders:** Files default to `FlashDocs/<format>/flashdoc_YYYY-MM-DD_hh-mm.ext` and can switch to first-line or custom patterns. Conflicts are uniquified automatically. 【F:service-worker.js†L249-L344】
...
2. **Trigger a save:**
   - Right-click → ⚡ **FlashDoc** → pick Auto, TXT, MD, YAML, Python, Label, etc. 【F:service-worker.js†L4-L69】【F:service-worker.js†L124-L167】
...
3. Create an upload zip excluding dev/output artefacts:
   ```bash
   zip -r out/flashdoc.zip . -x 'out/*' '.git/*'
   ```
...
- Exercise context menus, keyboard shortcuts, popup buttons and the label flow on `test-flashdoc.html`. 【F:SMOKE-TESTS.md†L1-L35】
